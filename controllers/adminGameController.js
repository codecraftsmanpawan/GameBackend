...
const getOngoingGameDetails = async (req, res) => {
    try {
        // Find the ongoing games (assuming there's one ongoing game of each type)
        const blackWhiteGame = await Game.findOne({ mode: 'blackWhite', status: 'running' });
        const tenColorsGame = await Game.findOne({ mode: 'tenColors', status: 'running' });
